				...
				if(aktuellerPunkt.getX()+i>=minX&&aktuellerPunkt.getX()+i<=maxX)
				{
					...
					if(aktuellerPunkt.getY()+j>=minY&&aktuellerPunkt.getY()+j<=maxY)
					{
						...
						for(AccessiblePoint ap:unusedPoints)
						{
							...
							{
								besucht=true;
								if(ap.getWeight()>(ip.getPixel((int)aktuellerPunkt.getX()+i,(int)aktuellerPunkt.getY()+j)+first.getWeight()))
								{
									ap.setWeight((ip.getPixel((int)aktuellerPunkt.getX()+i,(int)aktuellerPunkt.getY()+j)+first.getWeight()));
								}
							}
						}
						...
						if(besucht==false&&ready==false)
						{
							...
							AccessiblePoint ap=new AccessiblePoint(temp,(ip.getPixel((int)aktuellerPunkt.getX()+i,(int)aktuellerPunkt.getY()+j)+first.getWeight()),first);
							unusedPoints.add(ap);
						}
					}
				}

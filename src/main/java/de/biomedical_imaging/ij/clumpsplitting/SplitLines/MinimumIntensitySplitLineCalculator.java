...
public class MinimumIntensitySplitLineCalculator implements AbstractSplitLineCalculator
{
			...
			Clump c, ImageProcessor ip)
	{
		 ...
		 * wert; } } double maxi=0; for(int i=0;i<partialDerivative.length;i++)
		 * {
		 * 
		 * for(int j=0;j<partialDerivative[i].length;j++)
		 * {if(partialDerivative[i][j]>maxi) { maxi=partialDerivative[i][j]; }
		 * partialDerivative[i][j]=-partialDerivative[i][j]; } } maxi=maxi+1;
